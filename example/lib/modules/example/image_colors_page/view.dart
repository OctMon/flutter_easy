...
Widget buildView(
    ImageColorsState state, Dispatch dispatch, ViewService viewService) {
  void _onImagePickerClicked() async {
    ...
    if (selected == 0) {
      // 调用相机
      file = File(
          (await _picker.getImage(source: ImageSource.camera))?.path ?? "");
    } else if (selected == 1) {
      // 调用相册
      file = File(
  ...
  return BaseScaffold(
    appBar: BaseAppBar(
      title: BaseText(
          S.of(viewService.context).example_ExtractProminentColorsFromAnImage),
      actions: [
        BaseButton(
          child: Icon(
            Icons.add,
    ...
    body: state.file != null && state.paletteGenerator != null
        ...
        : Center(child: BaseText("Selected Image")),
  );
}
